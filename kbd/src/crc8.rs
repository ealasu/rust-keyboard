...
lazy_static! {
    pub static ref TABLE: [u8; 256] = {
      let mut table = [0; 256];
      let mut crc: u16 = 0;
      for i in 0..256 {
        crc = i;
        for j in 0..8 {
          crc = (crc << 1) ^ (if (crc & 0x80) != 0 { DI } else { 0 });
        }
        table[i as usize] = (crc & 0xFF) as u8;
      }
      table
    };

    pub static ref TABLE_MAXIM_CALC: [u8; 256] = make_table(0x31);
}
...
pub fn crc8(mut crc: u8, m: u8) -> u8 {
    TABLE_MAXIM[(crc ^ m) as usize]
}

pub fn crc8_buf(buf: &[u8]) -> u8 {
    let mut crc = 0;
    for &v in buf.iter() {
        crc = crc8(crc, v);
    }
...
mod tests {
    ...
    #[test]
    fn test_maxim() {
        assert_eq!(crc8_maxim(&[0x12]), 0x21);
        assert_eq!(crc8_maxim(&[0xFF, 0x00, 0x07, 0x5F, 0x07, 0xB7, 0x00, 0x68]),
                   0x37);
    }

    #[test]
    fn test_me() {
        assert_eq!(crc8(0, 0x12), 0x21);
        assert_eq!(crc8_buf(&[0xFF, 0x00, 0x07, 0x5F, 0x07, 0xB7, 0x00, 0x68]),
                   0x37);
    }

    #[test]
    fn test_table_maxim() {
        assert_eq!(&TABLE_MAXIM_CALC[..], &TABLE_MAXIM[..]);
    }
}
